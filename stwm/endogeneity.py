...
def iv_regression(Y: np.ndarray, X: np.ndarray, Z: np.ndarray) -> dict:
    ...
    # First-stage F-statistic (instrument strength, rule of thumb: F > 10)
    # Regress last column of X (endogenous WY) on instruments Z
    endog = X[:, -1]
    gamma_1st = np.linalg.lstsq(Z, endog, rcond=None)[0]
    fitted_1st = Z @ gamma_1st
    resid_1st = endog - fitted_1st
    SSR_1st = float(resid_1st @ resid_1st)
    SST_1st = float(np.sum((endog - endog.mean())**2))
    R2_1st = 1.0 - SSR_1st / SST_1st if SST_1st > 1e-15 else 0.0
    k_instr = Z.shape[1]
    # F = (R2/k_instr) / ((1-R2)/(N-k_instr))
    first_stage_F = (R2_1st / k_instr) / ((1.0 - R2_1st + 1e-15) / (N - k_instr)) if N > k_instr else np.nan

    return dict(beta=beta, se=se, t_stats=t_stat, p_values=pvals,
                residuals=resid, sigma2=sigma2, cov=cov,
                first_stage_F=float(first_stage_F), R2_first_stage=float(R2_1st))
